...
Test Case Link : https://dev.azure.com/itron/SoftwareProducts/_workitems/edit/2064760
===================================================================================================
Test Case      : 2064760
Description    : Verify Agent requested Exceeded Upstream daily data limit and the Policy Violation is recorded in PolicyViolationStatistics table 
...
===================================================================================================
Step 1 - 
Verify Agent requested Exceeded Upstream daily data limit and the Policy Violation is recorded in
PolicyViolationStatistics  table

Step 2 - 
 AgentId     FeatureID     ConstraintID 5    ViolationParameter  \"Daily Data Overage: [%] Bytes\"
ViolationCount
...
# AUTOGENERATED Test Case 2064760

# @pytest.mark.skip(reason="TODO: unimplemented test case")
@pytest.mark.regress_nightly
# @pytest.mark.regress_smoke
@pytest.mark.suite_id("2060761")
@pytest.mark.test_case("2064760")
def test_case(preinstalled_meter, logger, workdir):
    logger.trace("Executing Test Case 2064760 - Verify Agent requested Exceeded Upstream daily data limit and the Policy Violation is recorded in PolicyViolationStatistics table ")
    ...
    new_word='<Upstream dailyAlarmLimit="100" dailyDataLimit="10"/>'
    ...
    try:
        ...
        if initial_max_timestap[0] != '':
            query = f'select ViolationParameter from PolicyViolationStatistics where agentid = {agent_id} and  timestamp > {initial_max_timestap[0]} and ConstraintID = 5 order by timestamp desc limit 1'
        else:
            query = f'select ViolationParameter from PolicyViolationStatistics where agentid = {agent_id} and ConstraintID = 5 order by timestamp desc limit 1'

        time_out=time.time() + 2*60
        c = 0
        while time_out>time.time():
            ViolationParameter = preinstalled_meter.sql_query(query)
            value = ViolationParameter and int(ViolationParameter[0]) == 10
            if value :
                break 
            else:
                c+=1
        ...
        logger.trace('Step 2')
        assert value,"There is no violation register in policyviolationstatistics with constraint id 5"
